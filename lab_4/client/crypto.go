...
func createMessage(data, iv, key []byte) Message {
	msg := Message{
		Header: Header{
			ExternalKeyIdFlag: "",
			Version:           "",
	...
	msg.Header.ExternalKeyIdFlag = "1"
	msg.Header.Version = "0"
	msg.Header.CS = "111000"      //56 Ч+Н+О
	msg.Header.KeyId = "10000000" //128 не используется
	msg.Header.SeqNum = strconv.FormatUint(SeqNum, 10)
	...
	cipher := getBt(magic(t1, t2, t3))
	msg.PayloadData = hex.EncodeToString(cipher)
	h := make([]uint8, 32)
	...
	msg.ICV = hex.EncodeToString(h)
	SeqNum++
	return msg
}

func printArr(a []uint8, nums int) error {
