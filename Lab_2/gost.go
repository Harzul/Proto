...
var IV = make([]uint8, 64)
var N = make([]uint8, 64)
var Sigma = make([]uint8, 64)

var N_512 = make([]uint8, 64)
var N_0 = make([]uint8, 64)

func print_arr(a []uint8, len int) {
	for i := range len {
		if a[i] < 0x10 {
			fmt.Printf("0%x", a[i])
		} else {
...
func alg(message []uint8, len int, h []uint8) {
	...
	//fmt.Printf("paddedMes=");
	//print_arr(paddedMes,64);
...
func test_stribog() {
	...
	fmt.Printf("stribog tests\n")

	get512(m63, 63, h1)
	fmt.Printf("test1:\n\t")
	print_arr(h1, 64)
	if cmp(h1, test1, 64) == 0 {
		fmt.Printf("test1 failed\n")
		return
	}
	...
	if cmp(h2, test3, 32) == 0 {
		fmt.Printf("test4 failed\n")
		return
	}

	fmt.Printf("tests valid\n")
}
